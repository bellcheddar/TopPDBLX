...
#
# Ported from chatPDB unchanged in substance: it is the version that carries the Spotlight
# daemon-family detection, added there after Spotlight starved a training subprocess of CPU for
# 3+ hours on four separate occasions in one day. The relevant directory here is
# data/raw/mmcif_archive/, which holds 257,179 files and is exactly the kind of freshly-added
# tree that triggers a re-index.
# Run with: bash scripts/preflight.sh
# Then start training with: ./run.sh models.train_slm

set -e

echo "=== TopPDBLX Pre-Training Memory Flush ==="
echo ""
...
# Real incident, 2026-07-21 (chatPDB Phase 4, ported here unchanged): sudo mdutil fails silently with no TTY (agent
...
# data/raw/mmcif_archive/ has 256,444 newly-added files (353 GB) -- a very plausible repeated re-index
...
# Disk report (chatPDB-specific: data/raw/mmcif_archive/ is large and can eat headroom fast)
echo ""
echo "=== Disk Report ==="
df -H / | tail -1
echo ""
