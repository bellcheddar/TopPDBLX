...
# which is R1's ceiling and not its goal. This sweeps residual resolution across checkpoints so
# the decision to extend to a full epoch is made on evidence rather than on the loss curve.
#
# Reading the result:
#   resolution still climbing at the last checkpoint -> extend, resume with --iter-offset
#   resolution flat, JSON mostly invalid             -> capacity limited, raise LoRA rank
#   resolution flat, JSON valid but names unknown    -> lexicon limited, more training cannot help
#
...
for iter in "$@"; do
  ...
  ./run.sh models.eval_slm --checkpoint "$iter" --limit "$LIMIT" \
    ...
    | grep -E "resolution|schema valid|exact JSON|fully resolved" || true
done
...
print(f"{'iter':>6}  {'fidelity exact':>14}  {'resid valid':>11}  {'resid resolution':>16}  "
      f"{'resid full':>10}")
for it, d in rows:
    print(f"{it:>6}  {d['fidelity_exact_match_pct']:>13.2f}%  "
          f"{d['residual_schema_valid_pct']:>10.2f}%  "
          f"{d['residual_component_resolution_pct']:>15.2f}%  "
          f"{d['residual_fully_resolved_records_pct']:>9.2f}%")
if len(rows) >= 2:
    delta = (rows[-1][1]['residual_component_resolution_pct']
             - rows[-2][1]['residual_component_resolution_pct'])
    print(f"\nresolution change over the last interval: {delta:+.2f} points")
